...
export default function PublicRegisterPage() {
  ...
  const onSubmit = async (values) => {
    try {
      const result = await submitPublicRegistration({ ...values, declaration: Boolean(values.declaration) });
      setSuccess(result);
    } catch (err) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <section className="card p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs text-slate-400">Branch and Section *</label>
            <input className="input mt-1" {...register("teamLeaderBranchSection", { required: "Required" })} />
          </div>
          <div>
      ...
      {MEMBER_NUMBERS.slice(0, memberCount).map((n) => (
        <section key={n} className="card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs text-slate-400">Branch and Section</label>
              <input className="input mt-1" {...register(`member${n}BranchSection`)} />
            </div>
            <div>
              <label className="text-xs text-slate-400">Year</label>
